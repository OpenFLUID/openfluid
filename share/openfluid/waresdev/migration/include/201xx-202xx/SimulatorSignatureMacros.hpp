...
#define DECLARE_DOMAIN(domain) \
  if(!std::string(domain).empty()) Signature->Tags.push_back("domain::" + std::string(domain));

/**
  Macro for declaration of the simulator application processes
*/
#define DECLARE_PROCESS(process) \
  if(!std::string(process).empty()) Signature->Tags.push_back("process::" + std::string(process));

/**
  Macro for declaration of the simulator application numerical method
*/
#define DECLARE_METHOD(method) \
  if(!std::string(method).empty()) Signature->Tags.push_back("method::" + std::string(method));
