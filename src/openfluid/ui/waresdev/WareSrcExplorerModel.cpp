    ...
    QFileSystemModel(), m_DirPath(Path)
{
  ...
  m_DefaultIcons["*.f;*.F;*.for;*.FOR;*.f90;*.F90;*.fpp;*.FPP;*.f95;*.F95"] =
      ":/ui/common/icons/fortran.png";
...
QVariant WareSrcExplorerModel::data(const QModelIndex& Index, int Role) const
{
  ...
  if (Role == Qt::FontRole && !isDir(Index)
      && fileInfo(Index).suffix() == "cpp")
  {
    openfluid::waresdev::WareSrcManager::PathInfo PInfo = m_PathInfos.value(
        filePath(Index));

    if (openfluid::waresdev::WareSrcContainer(PInfo.m_AbsolutePathOfWare,
                                              PInfo.m_WareType,
                                              PInfo.m_WareName).getMainCppFile()
        == filePath(Index))
    {
      QFont Font;
      Font.setBold(true);
