...
MultiInjectGenerator::MultiInjectGenerator(const std::string& Selection) : Generator(),
  m_DataFile(""), m_Selection(Selection)
{
  std::vector<openfluid::tools::ClassIDVar> SelectionTriplets = 
    openfluid::tools::stringSelectionToClassIDVarList(m_Selection);

  for (const auto& Triplet : SelectionTriplets)
  {
    if (m_VarsByUnitsClass.find(Triplet.UnitsClassesStr) == m_VarsByUnitsClass.end())
    {
      m_VarsByUnitsClass[Triplet.UnitsClassesStr] = {};
    }
    m_VarsByUnitsClass[Triplet.UnitsClassesStr].insert(Triplet.VariablesStr);
  }
}
...
void MultiInjectGenerator::initParams(const openfluid::ware::WareParams_t& Params)
{
  ...
  {
    if (DataFileGivenPath.at(0) == '/')  // absolute path
    {
