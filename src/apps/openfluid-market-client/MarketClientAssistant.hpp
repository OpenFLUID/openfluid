...
class MarketClientAssistant : public Gtk::Assistant
{
  ...
  private:
    ...
    Gtk::Label m_AvailPacksLabel;
    Gtk::TreeView m_AvailPacksTreeview;

    Glib::RefPtr<Gtk::ListStore> m_RefAvailPacksModel;

    class AvailPacksColumns : public Gtk::TreeModel::ColumnRecord
    {
      public:
        ...
        AvailPacksColumns() { add(m_Selected); add(m_ID); add(m_Format); add(m_Name); }
    };

    AvailPacksColumns m_AvailPacksColumns;
