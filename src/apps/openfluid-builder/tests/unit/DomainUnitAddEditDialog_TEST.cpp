...
struct init_Dialog
{
    ...
    init_Dialog()
    {
      ...
      mp_Dialog = new DomainUnitAddEditDialogSub(*mp_AddDialog);

      std::string Path = CONFIGTESTS_INPUT_DATASETS_DIR
          + "/OPENFLUID.IN.Primitives";
      mp_EngProject = new EngineProject(Path);

      mp_Dialog->setEngineRequirements(mp_EngProject->getCoreRepository());
    }
...
BOOST_AUTO_TEST_CASE(test_initialize)
{
  ...
  mp_Dialog->setUnit(mp_EngProject->getCoreRepository().getUnit("ParentTestUnits",2));
  mp_Dialog->initEditionMode();
