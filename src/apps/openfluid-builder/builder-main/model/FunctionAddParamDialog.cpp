    ...
    openfluid::machine::ModelItemSignatureInstance* Sign)
{
  ...
  // create a dummy item to store together (only for "onChange" tests) parameters that are set and parameters from signatures
  // signature parameters first (because they have no value), then effectively set parameters (that can overwrite signature ones)
