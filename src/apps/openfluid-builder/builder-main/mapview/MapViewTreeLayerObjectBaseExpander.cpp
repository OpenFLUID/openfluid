...
/**
 \file MapViewTreeLayerObjectBaseExpander.cpp
 \brief Implements ...
...
MapViewTreeLayerObjectBaseExpander::MapViewTreeLayerObjectBaseExpander()
{
  ...
  mp_AdjustmentHScale = Gtk::manage(new Gtk::Adjustment(0.0, 0.0, 101.0, 1.0, 1.0, 1.0));
  mp_AdjustmentSpinButton = Gtk::manage(new Gtk::Adjustment(1.0, 1.0, 100.0, 1.0, 1.0, 0.0));
  ...
  mp_ColorButton->signal_color_set().connect(
      sigc::mem_fun(*this, &MapViewTreeLayerObjectBaseExpander::onColor));
  ...
  mp_MainTableExpander->attach(*mp_Separator, 0, 2, 1, 2,
      Gtk::FILL | Gtk::EXPAND, Gtk::SHRINK);
  mp_Separator = Gtk::manage(new Gtk::HSeparator());
  mp_MainTableExpander->attach(*mp_Separator, 0, 2, 3, 4,
      Gtk::FILL | Gtk::EXPAND, Gtk::SHRINK);
  mp_Separator = Gtk::manage(new Gtk::HSeparator());
  mp_MainTableExpander->attach(*mp_Separator, 0, 2, 5, 6,
      Gtk::FILL | Gtk::EXPAND, Gtk::SHRINK);
...
Gtk::Widget * MapViewTreeLayerObjectBaseExpander::asWidget()
{
...
MapViewTreeLayerObjectBaseExpander::mtype_Signal MapViewTreeLayerObjectBaseExpander::signalColor()
{
  return msigc_signalColor;
}

// =====================================================================
// =====================================================================

void MapViewTreeLayerObjectBaseExpander::onColor()
{
  m_Color = mp_ColorButton->get_color();
  setAlphaPercent();
  m_SizeLine = mp_SpinButton->get_value();
  mp_SpinButton->update();
  msigc_signalColor.emit(m_Id, m_Position, m_SizeLine, m_Color.get_red_p(), m_Color.get_green_p(), m_Color.get_blue_p(), m_Alpha);
}
...
void MapViewTreeLayerObjectBaseExpander::randColor()
{
  int r = (int) ((float)65536 * (float)rand() / (float)RAND_MAX);
  m_Color.set_red(r);
  int g = (int) ((float)65536 * (float)rand() / (float)RAND_MAX);
  m_Color.set_green(g);
  int b = (int) ((float)65536 * (float)rand() / (float)RAND_MAX);
  m_Color.set_blue(b);
}
...
void MapViewTreeLayerObjectBaseExpander::setAlphaPercent()
{
...
void MapViewTreeLayerObjectBaseExpander::setId(int Id)
{
  m_Id = Id;
}
...
void MapViewTreeLayerObjectBaseExpander::setPosition(int Position)
{
  m_Position = Position;
}
...
int MapViewTreeLayerObjectBaseExpander::getId()
{
  return m_Id;
}
...
int MapViewTreeLayerObjectBaseExpander::getPosition()
{
  return m_Position;
}
...
void MapViewTreeLayerObjectBaseExpander::setSizeLine(int SizeLine)
{
  m_SizeLine = SizeLine;
}
...
int MapViewTreeLayerObjectBaseExpander::getSizeLine()
{
  return m_SizeLine;
}
