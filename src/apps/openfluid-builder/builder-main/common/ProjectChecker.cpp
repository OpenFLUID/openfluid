...
void ProjectChecker::checkModelRequirements()
{
      ...
      Items.begin(); it != Items.end(); ++it)
  {
    ...
    // check Generators UnitClass
    if ((*it)->isType(openfluid::fluidx::ModelItemDescriptor::Generator))
    {
      openfluid::fluidx::GeneratorDescriptor* Gen =
          dynamic_cast<openfluid::fluidx::GeneratorDescriptor*>(*it);
      if (!Domain.isClassNameExists(Gen->getUnitClass()))
        ProjectMsg += "- Unit class " + Gen->getUnitClass()
                      + " doesn't exist for variable generated by " + ID + "\n";
    }
