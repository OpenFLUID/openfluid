...
MACRO(OPENFLUID_ADD_OBSERVER _OBSNAME)
  ...
  IF (NOT ${_OBSNAME}_SOURCE_DIR)
    SET(${_OBSNAME}_SOURCE_DIR ${CMAKE_CURRENT_SOURCE_DIR})
  ENDIF()  
  ...
  LINK_DIRECTORIES(${OpenFLUID_LIBRARY_DIRS} ${${_OBSNAME}_LIBRARY_DIRS}) 
  ...
  TARGET_LINK_LIBRARIES(${${_OBSNAME}_TARGET} ${OpenFLUID_LIBRARIES} ${${_OBSNAME}_LINK_LIBS})


  # ====== tests ======  

  IF(${_OBSNAME}_TESTS_DATASETS)
  
    ENABLE_TESTING()
  
    SET(_TESTS_OBS_SEARCHPATHS "${PROJECT_BINARY_DIR}")
  
    IF (${_OBSNAME}_TESTS_OBS_SEARCHPATHS)
      SET(_TESTS_OBS_SEARCHPATHS "${PROJECT_BINARY_DIR}")
    ENDIF()
    ...
    FOREACH(_CURRTEST ${${_OBSNAME}_TESTS_DATASETS})
      ADD_TEST(${${_OBSNAME}_ID}-${_CURRTEST} "${OpenFLUID_CMD_PROGRAM}"
                                "-i" "${${_OBSNAME}_SOURCE_DIR}/tests/${_CURRTEST}.IN"
                                "-o" "${_TESTS_OUTPUT_DIR}/${_CURRTEST}.OUT"
                                "-n" "${_TESTS_OBS_SEARCHPATHS}")
      MESSAGE(STATUS "  Added test ${${_OBSNAME}_ID}-${_CURRTEST}")                            
    ENDFOREACH()
  ENDIF()  
